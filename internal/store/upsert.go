...
func (s *Store) Upsert(l model.Listing, now time.Time) (UpsertResult, error) {
	...
	case errors.Is(err, sql.ErrNoRows):
		res, insertErr := tx.Exec(
			...
			string(l.Verdict), string(reason), l.Fingerprint, utcOrNil(l.PublishedAt), now, now)
		if insertErr != nil {
	...
	default:
		if _, updateErr := tx.Exec(
			`UPDATE listings SET url = ?, title = ?, raw_text = ?, bike = ?, variant = ?,
             ...
             verdict = ?, verdict_reason = ?, fingerprint = ?, status = 'active',
			...
			l.Fingerprint, utcOrNil(l.PublishedAt), now, id); updateErr != nil {
