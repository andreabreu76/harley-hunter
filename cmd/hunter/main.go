...
func collect(ctx context.Context, cfg config.Config, db *store.Store, dir string) error {
	...
	refreshFipe(db, cfg.Match.Years)
	sendAlerts(cfg, db)
	return nil
}
...
func refreshFipe(db *store.Store, years []int) {
	stored, err := fipe.Refresh(context.Background(), fipe.NewHTTPFetcher(), db, time.Now(), years)
	if err != nil {
